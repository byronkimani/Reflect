...
5. **No UI without a design reference.** Do not invent new UI layouts or visual treatments unless the user has provided an explicit Figma reference or approved direction. Written design specs in `docs/design/` are not maintained (pending a future redesign).
...
- [ ] If sending out a new feature, ensure release notes are documented.
